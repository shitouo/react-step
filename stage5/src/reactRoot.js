...
function getChildFiber(currentElement, parentFiberNode, isUpdate) {
    ...
    if (isUpdate) {
        // 更新过程
        // TODO 后面要把这里和下面合在一起
        let newFiberNode = createWorkInProgress(currentElement, currentElement.pendingProps);
        newFiberNode.return = parentFiberNode;
        // diff 并 标记updateTag
        const fiberTag = newFiberNode.tag;
        if (fiberTag === FIBERTAGS.ClassComponent) {
            // update classComponent
            
        } else if (fiberTag === FIBERTAGS.HostComponent) {
            // update hostComponent
        }
        // 解析children
        newFiberNode.child = getChildFiber(newFiberNode.child, newFiberNode, true);
    }
    // 深度优先遍历，生成Fiber tree
...
class ReactRoot {
    ...
    render(reactElement) {
        if (!reactElement) {
            ...
            this.rootFiber.child = getChildFiber(this.rootFiber.child, this.rootFiber, true);
        } else {
            // 初始装载
